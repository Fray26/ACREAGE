...
*Setup instructions for a gitian build of MASTERDEMO using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MASTERDEMO
...
- [Building MASTERDEMO](#building-masterdemo)
...
Clone the git repositories for masterdemo and gitian and then checkout the masterdemo version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/masterdemo-crypto/masterdemo.git
cd masterdemo
...
Follow the instructions in [doc/release-process.md](release-process.md) in the masterdemo repository
...
To build MASTERDEMO (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the masterdemo repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/masterdemo/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/masterdemo-crypto/masterdemo
...
For example:
```bash
URL=https://github.com/masterdemocoin/masterdemo.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit masterdemo=${COMMIT} --url masterdemo=${URL} ../masterdemo/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit masterdemo=${COMMIT} --url masterdemo=${URL} ../masterdemo/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit masterdemo=${COMMIT} --url masterdemo=${URL} ../masterdemo/contrib/gitian-descriptors/gitian-osx.yml
```
...
and follow the steps in the build process as normal.
    ...
    gpg: skipped "masterdemocoin": secret key not available

When you execute `gsign` you will get an error from GPG, which can be ignored. Copy the resulting `.assert` files
in `gitian.sigs` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/masterdemo-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/masterdemo-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/masterdemo-build.assert
```
...
[masterdemo/gitian.sigs](https://github.com/masterdemo-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
