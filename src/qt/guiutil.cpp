...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a MASTERDEMO address (e.g. %1)").arg("FHoujpfmyWqr69iWhv8D5kpenvSYwG1MGm"));
#endif
...
{
    // return if URI is not valid or is no MASTERDEMO: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::MasterDemo, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert masterdemo:// to masterdemo:
    //
    //    Cannot handle this later, because masterdemo:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::MasterDemo, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open masterdemo.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MASTERDEMO.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for MASTERDEMO.lnk
...
{
    return GetAutostartDir() / "masterdemo.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a masterdemo.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=MASTERDEMO\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the masterdemo app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add masterdemo app to startup item list
