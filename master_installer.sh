...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='masterdemo.conf'
CONFIGFOLDER='/root/.masterdemo'
...
function get_ip() {
  declare -a NODE_masterdemo
  for masterdemo in $(netstat -i | awk '!/Kernel|Iface|lo/ {print $1," "}')
  do
    NODE_masterdemo+=($(curl --interface $masterdemo --connect-timeout 2 -s4 icanhazip.com))
  done

  if [ ${#NODE_masterdemo[@]} -gt 1 ]
    then
      echo -e "${GREEN}More than one IP. Please type 0 to use the first IP, 1 for the second and so on...${NC}"
      INDEX=0
      for ip in "${NODE_masterdemo[@]}"
      do
        echo ${INDEX} $ip
        let INDEX=${INDEX}+1
      done
      read -e choose_ip
      NODEIP=${NODE_masterdemo[$choose_ip]}
  else
    NODEIP=${NODE_masterdemo[0]}
  fi
}
